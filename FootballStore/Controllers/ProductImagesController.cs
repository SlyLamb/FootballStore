    ...
    public class ProductImagesController : Controller
    {
        ...
        public ActionResult Upload(HttpPostedFileBase[] files)
        {
                ...
                if (files.Length <= 10)
                {
                    ...
                    if (allValid)
                    {
                        foreach (var file in files)
                        {
                            ...
                            {
                                ModelState.AddModelError("FileName", "Sorry an error occurred while saving the files to the disk, please try again.");
                            }
                        }
                    }
            ...
            if (ModelState.IsValid)
            {
                ...
                if (duplicates)
                {
                    ModelState.AddModelError("FileName", "All files uploaded except the files " + duplicateFiles + ", which already exist in the system." + " Please delete them and try again if you wish to overwrite them.");
                    return View();
                }
        ...
        private void SaveFileToDisk(HttpPostedFileBase file)
        {
            ...
            string strImg = System.IO.Path.GetFileName(file.FileName);
            string path = System.IO.Path.Combine(Server.MapPath(Constants.ProductImagePath), strImg);
            string thumbPath = System.IO.Path.Combine(Server.MapPath(Constants.ProductThumbnailPath), strImg);
